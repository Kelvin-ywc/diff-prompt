...
def train(cfg, local_rank, distributed, zero_shot, skip_optimizer_resume=False, save_config_path = None):
    ...
    if cfg.PROMPT.ENABLE:
        if cfg.PROMPT.TYPE == 'Mask-VPT':
            ...
            for k, p in model.named_parameters():
                if 'visual_prompts' in k or 'v2t' in k:
                    p.requires_grad = True
                else:
                    p.requires_grad = False
    ...
    for task_id, ft_cfg in enumerate(ft_configs, 1):
        for shuffle_seed in shuffle_seeds:
            ...
            if custom_shot == 10000:
                ...
                # load dit
                dit = load_dit(model_type='DiT-My/2', path='//workspace/prompt/PromptDiffusion/DiT-main/results/002-DiT-My-2-timestep100/checkpoints/0094200.pt', latent_size=28)
                diffusion_steps = 100
                diffusion = create_diffusion(timestep_respacing="ddim12", diffusion_steps=diffusion_steps, noise_schedule='squaredcos_cap_v2') 
                ...
                for iteration, (images, targets, idxs, positive_map, positive_map_eval, greenlight_map) in enumerate(data_loader):
                    ...
                    # )
                    samples, samples_latent_features = diffusion.ddim_sample_loop(
                        sample_fn, z.shape, z, clip_denoised=False, model_kwargs=dit_kwargs, progress=False, device=device, prompt_depth=4
                    )
                    # 保存图片
                    # 保存prompts
                    ...
                    for idx, sample in enumerate(samples_latent_features):
                        ...
                        sample = torch.clamp(255.0 * sample, 0, 255).squeeze(1).to("cpu", dtype=torch.uint8).numpy()
                        for i,s in enumerate(sample):
                            Image.fromarray(s).save(f"vis/sample_step_{idx}_{i}_{captions[i]}.png")
