...
def train(cfg, local_rank, distributed, zero_shot, skip_optimizer_resume=False, save_config_path = None):
        ...
        if cfg.PROMPT.TYPE == 'Mask-VPT':
            ...
            for k, p in model.named_parameters():
                if 'adapter' in k:
                    ...
                    # print(f'{k} is frozen')
                    p.requires_grad = False
        elif cfg.PROMPT.TYPE in ['FedTPG', 'FedTPG_v2']:
            for k, p in model.named_parameters():
    ...
    if True:
        import torchsummary
    # print(cfg.PROMPT.TYPE)

    optimizer = make_optimizer(cfg, model)
    # FIXME
    scheduler = None
    # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, 1, gamma=0.5, last_epoch=-1, verbose=False)
