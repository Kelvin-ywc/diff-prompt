...
def build_flickr_evaluator(cfg, dataset_name):
    evaluator = FlickrEvaluator(
        "DATASET/flickr30k/flickr30k/", # Hard written!!
        subset="test" if "test" in dataset_name  else "val",
...
def build_refexp_evaluator(coco, iou_types=["bbox"], img_ids=None):
    from maskrcnn_benchmark.data.datasets.refexp import RefExpEvaluator
    evaluator = RefExpEvaluator(coco, iou_types=iou_types, img_ids=img_ids)
    return evaluator
...
def inference(
    ...
    logger = logging.getLogger("maskrcnn_benchmark.inference")
    # logger.info(f"*************************************{cfg.DATASETS.REGISTER.train}*******************")
    dataset = data_loader.dataset
    if verbose:
        logger.info("Start evaluation on {} dataset({} images).".format(dataset_name, len(dataset)))
    start_time = time.time()
    ...
    if "flickr" in cfg.DATASETS.TEST[0]:
        evaluator = build_flickr_evaluator(cfg, dataset_name)
        ...
        evaluator = build_refexp_evaluator(dataset.coco, ['bbox'], img_ids=dataset.ids)
    else:
        evaluator = None
    ...
    for i, batch in enumerate(_iterator):
        if distributed:
            ...
            print(f'\nlocal_rank: {local_rank}, i: {i*num_local_ranks+local_rank}\n')
                ...
                for query_i in range(query_time):
                    ...
                    if False:
                        from thop import profile
                                # images, targets=None, captions=None, positive_map=None, greenlight_map=None
                        # import time
                        t1 = time.time()
                        model(images, captions=captions, positive_map=positive_map_label_to_token)
                        t2 = time.time()
                        flops, params = profile(model, inputs=(images, None, captions, positive_map_label_to_token, None))
                        logger.info(f'\n***********\nComputational Complexity: {flops}, Param: {params}\n**************\n')
                        print(f'\n***********\nComputational Complexity: {flops}, Param: {params}\n**************\n')
                        print(f'\n***********\Running Time: {t2-t1}\n**************\n')
                        exit(0)
        ...
        if visualizer is not None:
            ...
            if "lvis" in cfg.DATASETS.TEST[0]:
                ...
                visualizer_input = all_output[0]
            if distributed:
                image_id = dataset.ids[i*num_local_ranks+local_rank]
                assert len(dataset.ids)==256
            else:
            ...
            label_list.append(captions)
            visualizer.entities =  label_list
            
            threshold = visualize_param["threshold"]
            alpha = visualize_param["alpha"]
            box_pixel = visualize_param["box_pixel"]
            text_size = visualize_param["text_size"]
            text_pixel = visualize_param["text_pixel"]
            text_offset = visualize_param["text_offset"]
            text_offset_original = visualize_param["text_offset_original"]
            color = visualize_param["color"]
            infer_time_begin = time.time()
            # FIXME for gt
            if gt:
                # visualizer_input = targets[0]
                # sh = torch.tensor([targets[0].extra_fields['orig_size'][1], targets[0].extra_fields['orig_size'][0]], device=torch.device(0))
                sh = (targets[0].extra_fields['orig_size'][1], targets[0].extra_fields['orig_size'][0])
                t_len = targets[0].extra_fields['boxes'].shape[0]
                visualizer_input = BoxList(targets[0].extra_fields['boxes'], sh)
                # visualizer_input = BoxList(targets[0].extra_fields['boxes'], torch.tensor([224,224], device=torch.device(0)))
                visualizer_input.add_field("scores", torch.tensor([1.0 for i in range(targets[0].extra_fields['boxes'].shape[0])],device=torch.device(0)))
                visualizer_input.add_field("labels", torch.tensor([1 for i in range(targets[0].extra_fields['boxes'].shape[0])]))
                print(f'*************len:{t_len}**********************')
            ...
            print(f'\ninfer time: {infer_time_end - infer_time_begin}\n')
            if gt:
                folder_path = f'./visualize_gt_{cfg.DATASETS.TEST[0]}'
            else:
                folder_path = f'./visualize_{cfg.PROMPT.TYPE}_{cfg.DATASETS.TEST[0]}'
            if not os.path.exists(folder_path):
                os.makedirs(folder_path)
            if distributed:
                print(f'\nlocal_rank: {local_rank}, captions{captions}\n')
            if False:
                local_rank = int(os.environ["LOCAL_RANK"])

                # print(len(captions))
                imshow(result, f"{folder_path}/img_{i}_{captions[local_rank]}.jpg")
            else:
                write_img_begin = time.time()
                imshow(result, f"{folder_path}/img_{captions[0]}.jpg")
                write_img_end = time.time()
                print(f'write imge spend time: {write_img_end - write_img_begin}')
                # synchronize()
    ...
    if evaluator is not None:
        ...
        print(score)
        logger.info(f'interation: {i}')
        logger.info(score)
        import maskrcnn_benchmark.utils.mdetr_dist as dist
        # FIXME
        output_folder = f'./{cfg.OUTPUT_DIR}/flickr30k_prompt_diffusion_{i}_{dataset_name}'
