...
class ModulatedDataset(torchvision.datasets.CocoDetection):
    def __init__(self,
        ...
        if False:
            import random
            random.seed(42)
            unique_random_numbers = random.sample(range(1, 1000), 256)
            self.ids = [ids[i] for i in unique_random_numbers]
        elif False:
            # 假设文件名为 "numbers.txt"
            import random
            file_path = "./save_res/refcoco/r1.txt"

            # 读取文件中的数字并存储到列表中
            with open(file_path, 'r') as file:
                ids = [int(line.strip()) for line in file]
            random.seed(42)
            unique_random_numbers = random.sample(range(1, 10000), 256)
            self.ids = [ids[i] for i in unique_random_numbers]
        else:
            self.ids = ids
